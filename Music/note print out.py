...
def noteStateMatrixToMidi(statematrix, name="example", span=span):
    ...
    for time, state in enumerate(statematrix + [prevstate[:]]):
        ...
        for note in offNotes:
            pt = note+lowerBound
            noteoffevt = midi.NoteOffEvent(tick=(time - lastcmdtime) * tickscale, pitch=pt)
            track.append(noteoffevt)
            lastcmdtime = time
            # print(audio.midi2str(pt))
        for note in onNotes:
            pt = note + lowerBound
            track.append(midi.NoteOnEvent(tick=(time-lastcmdtime)*tickscale, velocity=40, pitch=pt))
            lastcmdtime = time
            # print(audio.midi2str(pt))
